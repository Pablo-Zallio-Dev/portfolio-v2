...
export default function Sidebar({ variant, onClose }: SidebarProps) {
  ...
  return (
    <aside
      ...
      lg:-translate-y-1/2
      flex flex-col ${variant === "desktop" ? (expanded ? "w-52 " : "w-18") : "w-69.5"} 
      py-3 shadow-xl/40 transition-all duration-500 bg-glass backdrop-blur-sm border border-border rounded-2xl `}
    >
      {variant === "mobile" && (
